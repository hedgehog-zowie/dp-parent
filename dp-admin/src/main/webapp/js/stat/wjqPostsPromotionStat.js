$(function(){
	var flag = $("#flag_h").val();
	var themeId = $("#themeId_h").val();
	var beginDate = $("#beginDate_h").val() ? $("#beginDate_h").val() : "";
	var endDate = $("#endDate_h").val() ? $("#endDate_h").val() : "";
	
	var params = "&flag=" + flag + "&statParams['themeId']=" + themeId + "&statParams['beginDate']=" + beginDate 
...
var actionManager = {
	initLink : function(params) {
		$.each($("a[name='navi_a']"), function(index, val){
			$(this).click(function(){
				...
				window.location.href = basePath+"dp/stat/wjqPostsPromotionStatView.action?page.currentPage=" + curPage + params;
			});
		});
	},
	initButton : function(params){
		...
		$("#btn_query").click(function(){
			$("#queryFrm").attr("action",basePath+"dp/stat/wjqPostsPromotionStatView.action");
			$("#queryFrm").submit();
		});
		
		$("#btn_exportExcel").click(function(){
			$("#queryFrm").attr("action",basePath+"dp/stat/wjqPostsPromotionStat2Excel.action");
			$("#queryFrm").submit();
		});
		...
		$("#goPageBtn").click(function(){
			...
			var url_pre = basePath+"dp/stat/wjqPostsPromotionStatView.action?page.currentPage=";
			if(page < 1) {
