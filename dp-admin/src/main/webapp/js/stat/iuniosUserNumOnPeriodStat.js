...
//	var flag = $("#flag_h").val();
	...
	var periodType = $("#periodType_h").val() ? $("#periodType_h").val() : "";
...
var actionManager = {
	initLink : function(params) {
		$.each($("a[name='navi_a']"), function(index, val){
			$(this).click(function(){
				...
				window.location.href = basePath+"dp/stat/iuniosUserNumOnPeriodStatView.action?page.currentPage=" + curPage + params;
			});
		});
	},
	initDialog : function(){
	...
	initButton : function(params,periodType){
		$("#flag_h").val(null);
		...
		$("#periodType").bind("change", function(){
			var value = this.value,
				format = value==="monthly"?'yyyy-MM':'yyyy-MM-dd';
			$("#beginDate").unbind("click").bind("click", function(){
				WdatePicker({skin:'whyGreen',dateFmt:format});
			});
			$("#endDate").unbind("click").click(function(){
				WdatePicker({skin:'whyGreen',dateFmt:format,minDate:'#F{$dp.$D(\'beginDate\',{M:0,d:0,m:1});}'});
			});
			$("#beginDate").val("");
			$("#endDate").val("");
		});
		...
		$("#btn_query").click(function(){
			...
			$("#queryFrm").attr("action",basePath+"dp/stat/iuniosUserNumOnPeriodStatView.action");
			$("#queryFrm").submit();
		});
		...
		$("#btn_exportExcel").click(function(){
			...
			$("#queryFrm").attr("action",basePath+"dp/stat/iuniosUserNumOnPeriodStat2Excel.action");
			$("#queryFrm").submit();
		});
		...
		$("#goPageBtn").click(function(){
			...
			var url_pre = basePath+"dp/stat/iuniosUserNumOnPeriodStatView.action?page.currentPage=";
			if(page < 1) {
