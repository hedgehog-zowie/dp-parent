	...
	initLink : function(params) {
		$.each($("a[name='navi_a']"), function(index, val){
			$(this).click(function(){
				...
				window.location.href = basePath+"dp/stat/queryIuniUserRegDailyStats.action?page.currentPage=" + curPage+params;
			});
		});
	},
	initButton : function(params){
		$("#btn_query").click(function(){
			$("#queryFrm").attr("action",basePath+"dp/stat/queryIuniUserRegDailyStats.action");
			$("#queryFrm").submit();
		});
		
		$("#btn_exportExcel").click(function(){
			$("#queryFrm").attr("action",basePath+"dp/stat/iuniUserRegDailyStats2Excel.action");
			$("#queryFrm").submit();
		});
		//分页
		...
		$("#goPageBtn").click(function(){
			...
			var url_pre = basePath+"dp/stat/queryIuniUserRegDailyStats.action?page.currentPage=";
			if(page < 1) {
