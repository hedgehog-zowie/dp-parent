$(function(){
	...
	var params = "&statParams['statRate']=" + statRate + "&statParams['appName']=" + appName + "&statParams['channels']=" + channels 
	...
	actionManager.renderChartData(flag,statRate,beginDate,endDate,appName,channels);
});
...
var actionManager = {
	initLink : function(params) {
		$.each($("a[name='navi_a']"), function(index, val){
			$(this).click(function(){
				...
				window.location.href = basePath+"dp/stat/gnAppActiveUserOfChannelStatView.action?page.currentPage=" + curPage + params;
			});
		});
	},
	initDialog : function(){
	...
	initButton : function(params, statRate){
		...
		$("#btn_query").click(function(){
			...
			$("#queryFrm").attr("action",basePath+"dp/stat/gnAppActiveUserOfChannelStatView.action");
			$("#queryFrm").submit();
		});
		...
		$("#btn_exportExcel").click(function(){
			...
			$("#queryFrm").attr("action",basePath+"dp/stat/gnAppActiveUserOfChannelStat2Excel.action");
			$("#queryFrm").submit();
		});
		...
		$("#goPageBtn").click(function(){
			...
			var url_pre = basePath+"dp/stat/gnAppActiveUserOfChannelStatView.action?page.currentPage=";
			if(page < 1) {
	...
	renderChartData : function(flag,statRate,beginDate,endDate,appName,channels){
		$.post(basePath+"dp/stat/getGnAppActiveUserOfChannelStatByJSON.action", 
			{
				"flag": flag,
