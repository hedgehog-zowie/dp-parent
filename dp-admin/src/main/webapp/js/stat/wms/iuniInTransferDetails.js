...
//	var flag = $("#flag_h").val();
	...
	var transferSale = $("#transferSale").val();
	var materialCode = $("#materialCode").val();

	var params = "&statParams['beginDate']=" + beginDate + "&statParams['endDate']=" + endDate
		+ "&warehouseName=" + warehouseName + "&transferSale=" + transferSale + "&materialCode=" + materialCode;
...
var actionManager = {
	initLink : function(params) {
		$.each($("a[name='navi_a']"), function(index, val){
			$(this).click(function(){
				...
				window.location.href = basePath+"dp/stat/iuniInTransferDetailsView.action?page.currentPage=" + curPage + params;
			});
		});
	},
	initButton : function(params){
		...
		$("#btn_query").click(function(){
			$("#queryFrm").attr("action",basePath+"dp/stat/iuniInTransferDetailsView.action");
			$("#queryFrm").submit();
		});
		
		$("#btn_exportExcel").click(function(){
			$("#queryFrm").attr("action",basePath+"dp/stat/iuniInTransferDetails2Excel.action");
			$("#queryFrm").submit();
		});
		...
		$("#goPageBtn").click(function(){
			...
			var url_pre = basePath+"dp/stat/iuniInTransferDetailsView.action?page.currentPage=";
			if(page < 1) {
